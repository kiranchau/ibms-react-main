...
import { validateFormData, workloadReportSchema } from '../../../Utils/validation';
...
const WorkloadReport = (props) => {
    ...
    const genrateReportBtnClickHandler = () => {
        ...
        validateFormData(workloadReportSchema, data).then(() => {
            setIsGenerating(true)
            generateReport("specialistWorkload", data).then((res) => {
                try {
                    ...
                    console.log("specialistWorkload-error: ", error)
                }
            }).catch((err) => {
                console.log("specialistWorkload-err-2: ", err)
            }).finally(() => {
    ...
    return (
        <div className="jobscode">
            <table>
                <tr>
                    <th>IB-RECRUIT Specialist Workload</th>
                </tr>
            </table>
            <div className='py-2 ps-3'>
                <div className='py-2'>
                    <b>Study Date Range </b>
                </div>
                ...
                <div className='py-2'>
                    <b> Agent(s) </b><span className='fw-lighter' style={{ fontSize: '0.9em' }}>(Leave Blank to generate a report with all agents.)</span>
                </div>
