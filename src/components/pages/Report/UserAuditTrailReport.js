...
const UserAuditTrailReport = (props) => {
    ...
    useEffect(() => {
        if (props?.users) {
            setUserList(sortByConcatenatedString(props.users, ["first_name", "last_name"]))
        } else {
            setUserList([])
        }
    }, [props?.users]);
...
export default UserAuditTrailReport;
