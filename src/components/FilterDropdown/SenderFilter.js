...
export default function SenderFilter({ setSelectedKeys, selectedKeys, confirm, clearFilters, close, filters, setFilters, onFilter, onReset }) {
    ...
    function getUsersList() {
        GetUsers().then((res) => {
            if (res.data?.users) {
                ...
                const sortedOptions = moveSelectedToTop(users, 'name', filters.sender_id ? filters.sender_id : [])
    ...
    function onFilterHandler() {
        ...
        onFilter()
        confirm()
        const sortedOptions = moveSelectedToTop(searchOptions, 'name', filters.sender_id ? filters.sender_id : [])
        setSearchOptions(sortedOptions);
    }

    // On Search Change function
    function onSelectHandler(e, item) {
        let checkedOpts = filters.sender_id ? filters.sender_id : []
        if (checkedOpts.includes(item.id)) {
            checkedOpts = checkedOpts.filter((opt) => { return opt != item.id })
        } else {
            checkedOpts.push(item.id)
        }
        setFilters({ ...filters, sender_id: checkedOpts })
    }
    ...
    return (
        <DropDownFilter
            ...
            selectedOptions={filters.sender_id}
            clearFilters={clearFilters}
            close={close}
