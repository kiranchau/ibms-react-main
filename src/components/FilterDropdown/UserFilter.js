...
export default function UserFilter({ setSelectedKeys, selectedKeys, confirm, clearFilters, close, filters, setFilters, onFilter, onReset }) {
    ...
    function getUsersList() {
        GetUsers().then((res) => {
            if (res.data?.users) {
                ...
                const sortedOptions = moveSelectedToTop(users, 'name', filters.user_id ? filters.user_id : [])
    ...
    function onFilterHandler() {
        ...
        onFilter()
        confirm()
        const sortedOptions = moveSelectedToTop(searchOptions, 'name', filters.user_id ? filters.user_id : [])
        setSearchOptions(sortedOptions);
    }

    // On Search Change function
    function onSelectHandler(e, item) {
        let checkedOpts = filters.user_id ? filters.user_id : []
        if (checkedOpts.includes(item.id)) {
            checkedOpts = checkedOpts.filter((opt) => { return opt != item.id })
        } else {
            checkedOpts.push(item.id)
        }
        setFilters({ ...filters, user_id: checkedOpts })
    }
    ...
    return (
        <DropDownFilter
            ...
            selectedOptions={filters.user_id}
            clearFilters={clearFilters}
            close={close}
